...
class MapApp:
    ...
    def _open_settings(self) -> None:
        ...
        ttk.Label(frame, textvariable=local_status_var, foreground="#607d8b").grid(
            row=9, column=0, sticky="w", pady=(2, 6)
        )

        local_button_row = ttk.Frame(frame)
        local_button_row.grid(row=10, column=0, sticky="ew")
        ttk.Button(
            local_button_row,
            text="Kiểm tra lại",
        ...
        ttk.Separator(frame, orient="horizontal").grid(row=11, column=0, sticky="ew", pady=14)
        ttk.Button(
            frame,
        ...
        ).grid(row=14, column=0, sticky="e", pady=(10, 0))
    ...
    def _accept_clipboard(self, text: str) -> None:
        ...
        self.previous = self.current
        self.current = position
        # Keep local/Npcap heading (and IslePilot fallback heading) when
        # live telemetry is available.
