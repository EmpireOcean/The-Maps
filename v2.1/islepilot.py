...
def verify_token(token: str) -> Optional[str]:
    """Blocking. Checks a manually-pasted overlay token against IslePilot's
    API and returns the account's steam_id, or None if the token is
    missing/invalid/expired or the request fails."""
    try:
        ...
        if response.status_code in (401, 403):
            return None
        response.raise_for_status()
        steam_id = str(response.json().get("steamId") or "")
