...
class _MovementTracker:
    def __init__(self) -> None:
        ...
        # Smallest payload_length actually seen from a confirmed movement
        # packet since the current lock started — a *running* floor rather
        # than the single most-recent sample's length, since Unreal's
        # delta-replication can also make packets shrink tick to tick (e.g.
        # when accelerating changes which fields need how many bits), not
        # just grow. Anchoring to one instantaneous length caused exactly
        # that shrink to look like non-movement traffic and get skipped.
        self._min_locked_payload_length: Optional[int] = None

    def reset(self) -> None:
        self._hypotheses.clear()
        self._current = None
        self._last_lock_update = 0.0
        self._min_locked_payload_length = None
    ...
    def try_track(
        ...
        # _decode() brute-forces every bit offset in the packet (expensive:
        # O(payload length in bits)) and used to run unconditionally on
        # every single UDP packet from every tracked game port — including
        # non-movement traffic (voice, heartbeats, ...) that can never
        # match. Once locked, a packet shorter than *any* movement packet
        # we've actually seen this lock can't contain enough bits for a
        # full position+rotation candidate, so it's safe to skip outright —
        # this is the majority of non-movement traffic (heartbeats and
        # voice frames are typically much smaller) and was what pegged a
        # CPU core continuously, starving the Tk main thread until the app
        # hung. Anything at or above that floor still goes through the real
        # decode: Unreal's delta-replication changes how many fields (and
        # bits) a packet carries tick to tick depending on what's dirty —
        # rotation changing fast alongside position, or speed changing
        # during acceleration — so the *locked* length isn't a single fixed
        # number; comparing against a floor that only ever widens (see
        # _min_locked_payload_length) instead of one instantaneous sample
        # is what keeps those legitimate variable-length packets from being
        # silently dropped.
        if (
            locked_and_fresh
            and self._min_locked_payload_length is not None
            and len(payload) < self._min_locked_payload_length
        ):
            return None
        ...
        if (
            ...
            if sample is not None:
                ...
                self._min_locked_payload_length = min(
                    self._min_locked_payload_length or sample.payload_length,
        ...
        if ready:
            ...
            self._last_known_at = observed_at
            self._min_locked_payload_length = winner.candidate.payload_length
            self._hypotheses.clear()
